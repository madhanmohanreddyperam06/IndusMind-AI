...
from app.modules.knowledge_extraction.constants import FAILURE_MODES


class FailureExtractor(BaseEntityExtractor):
    """Extractor for failure mode entities.
    
    Extracts failure modes like leakage, bearing failure, corrosion, overheating, etc.
    """
    
    def __init__(self):
        """Initialize the failure extractor."""
        super().__init__()
        self.failure_keywords = FAILURE_MODES.copy()
    
    def get_entity_type(self) -> EntityType:
        """Return the entity type."""
        return EntityType.FAILURE
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        """Extract failure entities from the context.
        
        Args:
            context: Extraction context
            
        Returns:
            List of extracted failure entities
        """
        entities = []
        text = context.text
        ...
        for keyword in self.failure_keywords:
            ...
            for match in matches:
                failure_name = match.group().strip()
                ...
                confidence = self._calculate_confidence(failure_name, context_text)
                
                entity = ExtractedEntity(
                    name=failure_name.title(),
                    entity_type=EntityType.FAILURE,
    ...
    def _calculate_confidence(self, failure_name: str, context: str) -> float:
        ...
        # Higher confidence if context contains failure-related words
        context_lower = context.lower()
        failure_context_keywords = ['failure', 'failed', 'fault', 'defect', 'problem', 'issue']
        if any(keyword in context_lower for keyword in failure_context_keywords):
            confidence += 0.2
        
        # Higher confidence if context contains equipment
        equipment_keywords = ['pump', 'valve', 'motor', 'equipment', 'system']
