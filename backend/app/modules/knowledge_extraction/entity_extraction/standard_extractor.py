...
class StandardExtractor(BaseEntityExtractor):
    """Extractor for standard entities (e.g., ASTM, ASME, API standards)."""
    ...
    def __init__(self):
        ...
        self.patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.standard_patterns]
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for pattern in self.patterns:
            ...
            for match in matches:
                ...
                confidence = self._calculate_confidence(standard, context_text)
                ...
                entity = ExtractedEntity(
                    ...
                    metadata={"standard": standard.upper()}
                )
    ...
    def _calculate_confidence(self, standard: str, context: str) -> float:
        confidence = 0.8
        context_lower = context.lower()
        std_keywords = ['standard', 'specification', 'spec', 'reference', 'code']
        if any(keyword in context_lower for keyword in std_keywords):
            confidence += 0.1
