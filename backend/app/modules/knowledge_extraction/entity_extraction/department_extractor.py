...
class DepartmentExtractor(BaseEntityExtractor):
    """Extractor for department entities."""
    
    def __init__(self):
        super().__init__()
        self.department_keywords = {
            'maintenance department', 'engineering department', 'operations department',
            'safety department', 'quality department', 'hse department', 'production department',
            'inspection department', 'procurement department', 'stores department',
            'electrical department', 'instrumentation department', 'mechanical department',
            'civil department', 'it department', 'hr department', 'finance department',
            'maintenance', 'engineering', 'operations', 'safety', 'quality', 'hse',
            'production', 'inspection', 'procurement', 'stores', 'electrical',
            'instrumentation', 'mechanical', 'civil', 'it', 'hr', 'finance'
        }
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for keyword in self.department_keywords:
            ...
            for match in matches:
                ...
                confidence = self._calculate_confidence(dept_name, context_text)
                ...
                entity = ExtractedEntity(
                    ...
                    metadata={"keyword": keyword, "matched_text": dept_name}
                )
    ...
    def _calculate_confidence(self, dept_name: str, context: str) -> float:
        confidence = 0.6
        context_lower = context.lower()
        dept_keywords = ['department', 'dept', 'section', 'division', 'unit']
        if any(keyword in context_lower for keyword in dept_keywords):
            confidence += 0.2
