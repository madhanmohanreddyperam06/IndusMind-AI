...
from app.modules.knowledge_extraction.constants import WORK_ORDER_PATTERNS


class WorkOrderExtractor(BaseEntityExtractor):
    """Extractor for work order entities."""
    
    def __init__(self):
        super().__init__()
        self.patterns = [re.compile(pattern, re.IGNORECASE) for pattern in WORK_ORDER_PATTERNS]
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for pattern in self.patterns:
            ...
            for match in matches:
                ...
                confidence = self._calculate_confidence(wo_number, context_text)
                
                entity = ExtractedEntity(
                    name=wo_number.upper(),
                    ...
                    metadata={"work_order": wo_number.upper()}
                )
    ...
    def _calculate_confidence(self, wo_number: str, context: str) -> float:
        confidence = 0.8
        context_lower = context.lower()
        wo_keywords = ['work order', 'maintenance', 'repair', 'wo', 'job']
        if any(keyword in context_lower for keyword in wo_keywords):
            confidence += 0.1
