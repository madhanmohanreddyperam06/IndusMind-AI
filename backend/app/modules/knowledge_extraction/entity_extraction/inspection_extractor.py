...
from app.modules.knowledge_extraction.constants import INSPECTION_TYPES


class InspectionExtractor(BaseEntityExtractor):
    """Extractor for inspection entities."""
    
    def __init__(self):
        super().__init__()
        self.inspection_keywords = INSPECTION_TYPES.copy()
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for keyword in self.inspection_keywords:
            ...
            for match in matches:
                inspection_name = match.group().strip()
                ...
                confidence = self._calculate_confidence(inspection_name, context_text)
                
                entity = ExtractedEntity(
                    name=inspection_name.title(),
                    ...
                    metadata={"keyword": keyword, "matched_text": inspection_name}
                )
    ...
    def _calculate_confidence(self, inspection_name: str, context: str) -> float:
        confidence = 0.7
        context_lower = context.lower()
        inspection_keywords = ['inspection', 'inspect', 'check', 'audit', 'survey']
        if any(keyword in context_lower for keyword in inspection_keywords):
            confidence += 0.2
