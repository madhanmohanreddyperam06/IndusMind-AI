"""Document reference entity extractor."""
import re
...
class DocumentReferenceExtractor(BaseEntityExtractor):
    """Extractor for document reference entities."""
    ...
    def __init__(self):
        ...
        self.patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.doc_patterns]
    
    def get_entity_type(self) -> EntityType:
        return EntityType.DOCUMENT_REFERENCE
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for pattern in self.patterns:
            ...
            for match in matches:
                ...
                confidence = self._calculate_confidence(doc_ref, context_text)
                
                entity = ExtractedEntity(
                    name=doc_ref.title(),
                    entity_type=EntityType.DOCUMENT_REFERENCE,
                    ...
                    metadata={"document_reference": doc_ref}
                )
    ...
    def _calculate_confidence(self, doc_ref: str, context: str) -> float:
        confidence = 0.6
        context_lower = context.lower()
        doc_keywords = ['document', 'reference', 'manual', 'procedure', 'sop']
        if any(keyword in context_lower for keyword in doc_keywords):
            confidence += 0.2
