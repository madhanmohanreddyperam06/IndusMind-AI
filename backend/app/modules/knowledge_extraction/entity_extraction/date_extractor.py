...
from app.modules.knowledge_extraction.constants import DATE_PATTERNS


class DateExtractor(BaseEntityExtractor):
    """Extractor for date entities."""
    
    def __init__(self):
        super().__init__()
        self.patterns = [re.compile(pattern, re.IGNORECASE) for pattern in DATE_PATTERNS]
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for pattern in self.patterns:
            ...
            for match in matches:
                ...
                confidence = self._calculate_confidence(date_str, context_text)
    ...
    def _calculate_confidence(self, date_str: str, context: str) -> float:
        confidence = 0.8
        context_lower = context.lower()
        date_keywords = ['date', 'scheduled', 'planned', 'completed', 'inspection', 'maintenance']
