...
from app.modules.knowledge_extraction.constants import QUALITY_TERMS


class QualityExtractor(BaseEntityExtractor):
    """Extractor for quality entities."""
    
    def __init__(self):
        super().__init__()
        self.quality_keywords = QUALITY_TERMS.copy()
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for keyword in self.quality_keywords:
            ...
            for match in matches:
                quality_name = match.group().strip()
                ...
                confidence = self._calculate_confidence(quality_name, context_text)
                
                entity = ExtractedEntity(
                    name=quality_name.title(),
                    ...
                    metadata={"keyword": keyword, "matched_text": quality_name}
                )
    ...
    def _calculate_confidence(self, quality_name: str, context: str) -> float:
        confidence = 0.7
        context_lower = context.lower()
        quality_keywords = ['quality', 'inspection', 'test', 'check', 'verify', 'validate']
        if any(keyword in context_lower for keyword in quality_keywords):
            confidence += 0.2
