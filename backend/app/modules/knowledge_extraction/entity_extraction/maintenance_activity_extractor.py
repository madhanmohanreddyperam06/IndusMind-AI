"""Maintenance activity entity extractor."""
import re
...
from app.modules.knowledge_extraction.constants import MAINTENANCE_ACTIVITIES


class MaintenanceActivityExtractor(BaseEntityExtractor):
    """Extractor for maintenance activity entities.
    
    Extracts maintenance activities like inspection, calibration, lubrication, etc.
    """
    
    def __init__(self):
        """Initialize the maintenance activity extractor."""
        super().__init__()
        self.activity_keywords = MAINTENANCE_ACTIVITIES.copy()
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        """Extract maintenance activity entities from the context."""
        entities = []
        text = context.text
        ...
        for keyword in self.activity_keywords:
            ...
            for match in matches:
                activity_name = match.group().strip()
                ...
                confidence = self._calculate_confidence(activity_name, context_text)
                
                entity = ExtractedEntity(
                    name=activity_name.title(),
                    entity_type=EntityType.MAINTENANCE_ACTIVITY,
                    ...
                    metadata={"keyword": keyword, "matched_text": activity_name}
                )
    ...
    def _calculate_confidence(self, activity_name: str, context: str) -> float:
        ...
        maintenance_keywords = ['maintenance', 'preventive', 'corrective', 'scheduled', 'routine']
