"""Person entity extractor using spaCy NLP."""
...
class PersonExtractor(BaseEntityExtractor):
    """Extractor for person entities using spaCy NER."""
    ...
    def _load_spacy_model(self):
        ...
        try:
            ...
            try:
                ...
                # Fallback to basic model if small model not available
                self.nlp = spacy.load("en_core_web_lg")
        except ImportError:
            print("spaCy not installed, PersonExtractor will not work")
    ...
    def extract(self, context: ExtractionContext) -> List[ExtractedEntity]:
        ...
        for ent in doc.ents:
            if ent.label_ == "PERSON":
                ...
                confidence = self._calculate_confidence(person_name, context_text)
    ...
    def _calculate_confidence(self, person_name: str, context: str) -> float:
        confidence = 0.7
        context_lower = context.lower()
        person_keywords = ['inspector', 'engineer', 'technician', 'operator', 'manager', 'supervisor']
        if any(keyword in context_lower for keyword in person_keywords):
            confidence += 0.1
